...
exports.isVisionOcrEnabled = isVisionOcrEnabled;
exports.analyzeDocumentImage = analyzeDocumentImage;
exports.detectLiveFaceBox = detectLiveFaceBox;
const vision_1 = require("@google-cloud/vision");
const FeatureType = vision_1.protos.google.cloud.vision.v1.Feature.Type;
let client = null;
function getVisionClient() {
    if (process.env.VISION_OCR_ENABLED === 'false')
        return null;
...
function avgPageConfidence(full) {
...
async function analyzeDocumentImage(buffer) {
...
async function detectLiveFaceBox(buffer) {
    const c = getVisionClient();
    if (!c)
        return null;
    const [batch] = await c.batchAnnotateImages({
