...
exports.similarityToMatchPercent = similarityToMatchPercent;
exports.buildDocumentPortraitTemplateFromNormalized = buildDocumentPortraitTemplateFromNormalized;
exports.buildDocumentPortraitTemplate = buildDocumentPortraitTemplate;
exports.buildLiveFaceTemplate = buildLiveFaceTemplate;
exports.matchDocumentTemplateToLive = matchDocumentTemplateToLive;
...
async function tryDecode(buffer) {
    return (0, sharp_1.default)(buffer, sharpLoose).rotate().toBuffer();
}
/**
...
async function decodeOnly(buffer) {
    if (!buffer?.length)
    ...
    try {
        ...
        try {
            return await (0, sharp_1.default)(buffer, sharpLoose)
...
async function capDecodedImageSize(buf) {
    const meta = await (0, sharp_1.default)(buf, sharpLoose).metadata();
    ...
    if (maxSide <= MAX_DECODE_SIDE && pixels <= MAX_DECODE_PIXELS)
        return buf;
    return (0, sharp_1.default)(buf, sharpLoose)
...
function similarityToMatchPercent(cos) {
    const t = (cos + 1) / 2;
    return Math.round(Math.min(100, Math.max(0, t * 100)));
}
async function rawGrayscaleVectorFromNormalized(normalized, crop) {
    const meta = await (0, sharp_1.default)(normalized, sharpLoose).metadata();
    const w = meta.width ?? 0;
    const h = meta.height ?? 0;
    if (w < 16 || h < 16)
        throw new Error('Image too small');
    let pipeline = (0, sharp_1.default)(normalized, sharpLoose);
    if (crop) {
    ...
    if (data.length < pixels * ch)
        throw new Error('Raw buffer size mismatch');
 ...
 */
function liveSelfiePortraitCrop(w, h) {
 ...
 */
async function buildDocumentPortraitTemplateFromNormalized(normalized, documentType) {
    const meta = await (0, sharp_1.default)(normalized, sharpLoose).metadata();
    const w = meta.width ?? 0;
    const h = meta.height ?? 0;
    if (w < 16 || h < 16)
        throw new Error('Image too small');
    const kind = documentType === 'RG' ? 'RG' : 'CNH';
...
async function buildDocumentPortraitTemplate(buffer, documentType) {
...
async function buildLiveFaceTemplate(buffer) {
    const normalized = await normalizeImageBuffer(buffer);
    const meta = await (0, sharp_1.default)(normalized, sharpLoose).metadata();
    const w = meta.width ?? 0;
    const h = meta.height ?? 0;
 ...
 */
async function matchDocumentTemplateToLive(docVec, liveImageBuffer) {
    ...
    if (w < 16 || h < 16)
        return 0;
    async function vecFrom(buf) {
        const m = await (0, sharp_1.default)(buf, sharpLoose).metadata();
        const ww = m.width ?? 0;
        const hh = m.height ?? 0;
    ...
    const flipped = await (0, sharp_1.default)(normalized, sharpLoose).flop().toBuffer();
...
function compareTemplates(doc, live) {
