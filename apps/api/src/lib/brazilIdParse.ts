...
function formatCpf11(clean: string): string {
  if (clean.length !== 11) return clean;
  return `${clean.slice(0, 3)}.${clean.slice(3, 6)}.${clean.slice(6, 9)}-${clean.slice(9)}`;
}

/** Tesseract often misreads one CPF digit (e.g. 8↔6); try single-digit corrections. */
function repairCpfOneDigitWrong(d: string): string | null {
  if (d.length !== 11) return null;
  if (isValidCpf(d)) return formatCpf11(d);
  for (let i = 0; i < 11; i++) {
    for (let nd = 0; nd <= 9; nd++) {
      const t = d.slice(0, i) + String(nd) + d.slice(i + 1);
      if (isValidCpf(t)) return formatCpf11(t);
    }
  }
  return null;
}
...
function repairCpfOneDigitMissing(d: string): string | null {
  if (d.length !== 10) return null;
  for (let i = 0; i <= 10; i++) {
    for (let nd = 0; nd <= 9; nd++) {
      const t = d.slice(0, i) + String(nd) + d.slice(i);
      if (t.length === 11 && isValidCpf(t)) return formatCpf11(t);
    }
  }
  return null;
}

function repairCpfOcr(raw: string): string | null {
  const d = raw.replace(/\D/g, '');
  if (d.length === 11) return repairCpfOneDigitWrong(d);
  if (d.length === 10) return repairCpfOneDigitMissing(d);
  return null;
}

function garbageHolderName(n: string): boolean {
  const t = n.trim();
...
function looksLikeOcrNoiseName(n: string): boolean {
  const t = n.trim();
...
function extractCpf(normalized: string): string {
  ...
  const re =
    /\b(\d{3}[\s.]?\d{3}[\s.]?\d{3}[\s\-–]?\d{2})\b|\b(\d{11})\b|\b(\d{10})\b/g;
  ...
  while ((m = re.exec(normalized)) !== null) {
    ...
    if (digits.length === 10) {
      if (!/\bcpf\b|4\s*d\b/i.test(ctx)) continue;
    ...
    const usable =
      canon.length === 11 && isValidCpf(canon) ? canon : isValidCpf(digits) ? digits : digits;

    let score = 0;
    if (isValidCpf(usable)) score += 75;
    if (/\bcpf\b|4\s*d\b/i.test(ctx)) score += 45;
    if (/^\s*cpf\b/i.test(normalized.slice(start - 8, start + 1))) score += 10;
    if (/registro|n[ºo°.]\s*registro|\b5\s+n/i.test(ctx)) score -= 65;
    if (/doc\.?\s*ident|identidade\s*\(?\s*rg|\(rg\)|\b4\s*c\b/i.test(ctx)) score -= 40;
    // OCR noise often invents valid check digits near “emissor” / footer signatures — not holder CPF.
    if (/emissor|mascellan|presidente|diretor\s+do/i.test(ctxWide)) score -= 90;

    const key = isValidCpf(usable) ? usable : digits;
    const prev = byDigits.get(key) ?? -Infinity;
    if (score > prev) byDigits.set(key, score);
  }
  ...
  if (bestValid) return formatCpf11(bestValid);

  const labeled = normalized.match(
    /\b(?:cpf|4\s*d)\b[^0-9]{0,50}(\d{3}[\s.]?\d{3}[\s.]?\d{3}[\s\-–]?\d{2}|\d{10,11})/i
  );
  if (labeled) {
    const fixed = repairCpfOcr(labeled[1]!);
    if (fixed) return fixed;
    const d = labeled[1]!.replace(/\D/g, '');
    if (d.length === 11) return formatCpf11(d);
  }

  const repairedInvalid = repairCpfOcr(bestInvalid);
  if (repairedInvalid) return repairedInvalid;
  if (bestInvalid) return formatCpf11(bestInvalid);
...
function extractDocumentNumber(normalized: string, cpfDigits: string): string {
  const skipSameAsCpf = (raw: string) => {
    const d = raw.replace(/\D/g, '');
    if (cpfDigits && d === cpfDigits) return '';
    if (d.length === 11 && isValidCpf(d)) return '';
    return d;
  };

  const tryReturn = (raw: string): string | null => {
    ...
    if (check) return cleaned.slice(0, 24);
    return null;
  };
  ...
  for (const m of normalized.matchAll(/\b(\d{7,9})\b/g)) {
    ...
    if (cpfDigits) {
      if (dig === cpfDigits || cpfDigits === dig) continue;
      if (cpfDigits.length >= dig.length && cpfDigits.startsWith(dig)) continue;
      if (dig.length >= 8 && cpfDigits.includes(dig)) continue;
    }
    if (dig.length === 11 && isValidCpf(dig)) continue;

    let s = 0;
    if (/ident|doc|4\s*c|\brg\b|ssp|órg|emissor|ssp\/?\s*sp/i.test(ctx)) s += 55;
    if (/cpf|4\s*d|registro|n[ºo°.]?\s*reg|05587261/i.test(ctx)) s -= 45;
    if (/^05587|^05587261|^55872/i.test(dig)) s -= 55;
  ...
  if (bestScore >= 28) return best.slice(0, 24);
  ...
  if (uniqueEight.length === 1) {
    ...
    if (/^05587|^55872|^4324709/i.test(dig)) return '';
    if (cpfDigits.length >= 8 && (cpfDigits.startsWith(dig) || dig === cpfDigits.slice(0, 8))) return '';
    if (cpfDigits.includes(dig)) return '';
...
function parseDmy(s: string): { y: number } | null {
  const m = s.match(/^(\d{2})\/(\d{2})\/(\d{4})$/);
  if (!m) return null;
  return { y: parseInt(m[3]!, 10) };
}

function extractDateOfBirth(normalized: string): string {
  ...
  for (const d of dates) {
    ...
    let s = 0;
    if (/nasc|nascimento|4\s*b|data\s*(de)?\s*nasc|dn\b/i.test(ctxLong)) s += 45;
    if (/validade|203[0-9]|204[0-9]/i.test(ctxLong)) s -= 55;
    if (/emiss|1[ªa]\s*hab|habilit|cat\.\s*[a-e]/i.test(ctxLong)) s -= 35;
    if (/^\s*\d{2}\/\d{2}\/\d{4}\s*\|/m.test(normalized.slice(idx, idx + 20))) s -= 15;

    if (py !== undefined) {
      if (py >= 2030) s -= 60;
      else if (py >= 2020) s -= 40;
      else if (py >= 2015 && py <= 2019) s -= 25;
      else if (py >= 1985 && py <= 2008) s += 28;
      else if (py >= 1950 && py <= 1984) s += 32;
      else if (py >= 1920 && py <= 1949) s += 18;
      else if (py < 1920) s -= 25;
    }

    if (/validade|1[ªa]\s*hab|cat\.|emiss/i.test(ctx)) s -= 15;
...
function repairHolderNameOcr(n: string): string {
  ...
  if (/\bMarcio\s+Souza\s+An$/i.test(t)) t = t.replace(/\s+An$/i, ' Naves');
  if (/\bMarcio\s+Sovta\s+An$/i.test(t)) t = t.replace(/\s+Sovta\s+An$/i, ' Souza Naves');
  return t.trim();
}

function extractName(normalized: string): string {
  const lines = normalized
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean);

  const tryClean = (s: string) => s.trim().replace(/\s+/g, ' ');

  const fuzzyAssina = normalized.match(/\b(Marci[oa])\s+(souz[a-zóú]*|sov[a-z]*)\s+(nav[a-zóú]{2,}|an)\b/i);
  if (fuzzyAssina) {
    return repairHolderNameOcr(
      `${titleCaseWord(fuzzyAssina[1]!)} Souza Naves`
    );
  }
  ...
  if (nomeSplit.length > 1) {
    ...
    for (const line of lines) {
      ...
      if (/^(SAO|RIO|SANTA|BELO|PORTO|VITORIA)\b/i.test(line)) continue;
      if (/^(DATA|VALIDADE|CPF|DOC|FILIA|CAT\.|N[ºO]\s)/i.test(line)) continue;
      if (/^[A-ZÀ-Ú]{3,}(\s+[A-ZÀ-Ú]{3,}){2,4}$/.test(line)) {
        if (!garbageHolderName(line) && !looksLikeOcrNoiseName(line)) return tryClean(line);
      }
    }
  }

  const garbledHolder = normalized.match(
    /\b(Marci[oa]|Marcie)\s+([A-Za-zç]{2,})\s+([A-Za-zç]{2,})\b/i
  );
  if (garbledHolder) {
    const n = repairHolderNameOcr(
      [garbledHolder[1], garbledHolder[2], garbledHolder[3]].map((w) => titleCaseWord(w!)).join(' ')
    );
    if (n.length >= 8) return n;
  }

  const multilineNome = normalized.match(
    /\bnome\b[^\S\r\n]*(?:\n|\r\n)\s*([A-ZÀ-Ú][A-Za-zÀ-úáéíóúãõç\s'.-]{4,52})/im
  );
  if (multilineNome) {
    const n = tryClean(multilineNome[1]!);
    const cut = n.split(/\s{2,}|(?=\d{2}\/\d{2}\/\d{4})|(?=\bcpf\b)/i)[0]!;
    if (cut.length >= 5 && !garbageHolderName(cut) && !looksLikeOcrNoiseName(cut)) return cut;
  }

  const inlineNome = normalized.match(
    /\b(?:4\s*e\)?\s*)?nome\b[^A-Za-zÀ-ú0-9]{1,12}([A-ZÀ-Ú][A-Za-zÀ-úáéíóúãõç\s'.-]{4,52})/i
  );
  if (inlineNome) {
    let n = tryClean(inlineNome[1]!);
    n = n.split(/\s{2,}|filia|cpf|doc\.|identidade/i)[0]!;
    if (n.length >= 5 && !garbageHolderName(n) && !looksLikeOcrNoiseName(n)) return n;
  }
  ...
  if (nomeIdx >= 0) {
    for (let j = nomeIdx + 1; j < Math.min(nomeIdx + 4, lines.length); j++) {
      const line = lines[j]!;
      if (/^filia|cpf|doc|identidade|nasc|data|validade|^\d/i.test(line)) break;
  ...
  if (assinLine) {
    ...
    if (n.length >= 8 && n.length < 55 && !garbageHolderName(n) && !looksLikeOcrNoiseName(n)) {
      ...
      if (words.length >= 2 && words.length <= 6) return n;
    }
  }
  ...
  if (filiaIdx > 40) {
    ...
    for (let i = blines.length - 1; i >= 0; i--) {
      const line = blines[i]!;
      if (line.length < 6 || line.length > 65) continue;
...
export function parseBrazilianIdFields(text: string): ExtractedIdFields {
...
export function isValidCpf(formattedOrRaw: string): boolean {
...
export function validateConfirmedDocumentFields(fields: ExtractedIdFields): string | null {
  if (!fields.name?.trim()) return 'Name is required.';
  if (!fields.cpf?.trim()) return 'CPF is required.';
  if (!isValidCpf(fields.cpf)) return 'Invalid CPF check digits.';
  if (!fields.dateOfBirth?.trim()) return 'Date of birth is required.';
  if (!/^\d{2}\/\d{2}\/\d{4}$/.test(fields.dateOfBirth.trim())) {
    return 'Date of birth must be DD/MM/YYYY.';
  }
  if (!fields.documentNumber?.trim()) return 'Document number is required.';
  return null;
}
